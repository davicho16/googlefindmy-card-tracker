...
`;
  ...
  class GoogleFindMyCardTracker extends HTMLElement {
    ...
    async _fetchHistory(entityId, range) {
      ...
      // significant_changes_only=0 le pide a HA que no filtre cambios "poco
      // significativos"; minimal_response=0/ausente hace que SI incluya los
      // atributos (lat/lon) de cada punto, no solo el estado. Sin esto, la
      // API devuelve entradas sin atributos y el filtro de abajo las descarta
      // todas.
      const path = `history/period/${start.toISOString()}?filter_entity_id=${encodeURIComponent(
        entityId
      )}&end_time=${encodeURIComponent(end.toISOString())}&no_attributes=false&significant_changes_only=0&minimal_response=0`;
      let result;
      try {
  ...
  window.customCards.push({
    ...
    description:
      "Mapa interactivo para Google Find My Device con inicio/fin, marcadores numerados, flechas de dirección, reproducción del recorrido, estadísticas, exportación GPX/KML, rango de fecha/hora personalizado y agrupamiento de paradas largas.",
    preview: true,
