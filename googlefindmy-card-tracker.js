...
`;
  ...
  // Prefijo usado para identificar las automatizaciones creadas por esta
  // card (para no listar/tocar otras automatizaciones del usuario que no
  // tengan nada que ver con esta funcionalidad). La automatización es la
  // fuente de verdad: las zonas de HA son una storage collection sin id
  // personalizable, así que no se les puede aplicar un prefijo propio.
  ...
  class GoogleFindMyCardTracker extends HTMLElement {
    ...
    async _loadGeofences() {
      ...
      // Las zonas viven en la storage collection de HA (API por WebSocket,
      // zone/create|update|delete|list), sin id personalizable: el id real
      // lo genera HA a partir del nombre. Por eso la automatización (cuyo id
      // SÍ controlamos, gfm_alert_<slug>) es la fuente de verdad para saber
      // qué geocercas son "nuestras"; la zona se resuelve a partir del
      // entity_id guardado en el meta JSON de la automatización.
      const automationEids = Object.keys(this._hass.states).filter((eid) => {
        if (!eid.startsWith("automation.")) return false;
        const attrs = this._hass.states[eid].attributes;
        return attrs && typeof attrs.id === "string" && attrs.id.startsWith(GFM_AUTOMATION_PREFIX);
      });
      const results = [];
      for (const automationEid of automationEids) {
        const automationId = this._hass.states[automationEid].attributes.id;
        const slug = automationId.slice(GFM_AUTOMATION_PREFIX.length);
        let meta = {};
        try {
          const cfg = await this._hass.callApi("GET", `config/automation/config/${automationId}`);
        ...
        const zoneEntityId = meta.zone || null;
        const zoneState = zoneEntityId ? this._hass.states[zoneEntityId] : null;
        const zoneId = meta.zoneId || (zoneEntityId ? zoneEntityId.slice("zone.".length) : null);
        results.push({
          slug,
          zoneEntityId,
          zoneId,
          automationEntityId: automationEid,
          automationId,
          name:
            (zoneState && zoneState.attributes && zoneState.attributes.friendly_name) ||
            meta.name ||
            slug,
          lat: zoneState && zoneState.attributes ? zoneState.attributes.latitude : meta.lat != null ? meta.lat : null,
          lon: zoneState && zoneState.attributes ? zoneState.attributes.longitude : meta.lon != null ? meta.lon : null,
          radius:
            zoneState && zoneState.attributes ? zoneState.attributes.radius : meta.radius != null ? meta.radius : null,
          meta,
        });
      }
    ...
    _buildAutomationConfig(draft, slug, zoneEntityId, zoneId) {
    ...
    async _saveGeofenceForm() {
      ...
      const existing = isEdit ? this._geofences.find((g) => g.slug === slug) : null;

      try {
        // Las zonas de HA son una "storage collection" que se administra por
        // WebSocket (zone/create, zone/update, zone/delete) — no existe el
        // endpoint REST config/zone/config/<id> (de ahí el 404). El id real
        // de la zona lo genera HA a partir del nombre y solo se conoce
        // DESPUÉS de crearla; por eso creamos/actualizamos la zona primero y
        // recién con ese resultado armamos la automatización.
        let zoneResult;
        if (isEdit && existing && existing.zoneId) {
          zoneResult = await this._hass.callWS({
            type: "zone/update",
        ...
        const zoneEntityId = `zone.${zoneId}`;

        await this._hass.callApi(
          "POST",
          `config/automation/config/${GFM_AUTOMATION_PREFIX}${slug}`,
          this._buildAutomationConfig(d, slug, zoneEntityId, zoneId)
        );
        try {
    ...
    async _deleteGeofence(slug) {
      ...
      try {
        ...
        if (g && g.zoneId) {
          await this._hass.callWS({ type: "zone/delete", zone_id: g.zoneId });
        }
        this._geofencesLoaded = false;
        await this._loadGeofences();
      } catch (e) {
