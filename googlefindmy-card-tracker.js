  ...
  async function ensureLeaflet() {
    ...
    if (!window.L.Symbol) {
      try {
        ...
        // Arrows are an enhancement only - keep going without them.
  ...
  function computeStats(points) {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      if (dtHours > 0) {
        ...
        if (speed > maxSpeed && speed < 300) maxSpeed = speed; // discard GPS glitches > 300km/h
      }
    }
  ...
  class GoogleFindMyCardTracker extends HTMLElement {
    ...
    set hass(hass) {
      ...
      if (first && this._config.entities.length > 0) {
        // auto-select first configured device
    ...
    _styles() {
      return `
        ...
        .gfm-body { display: flex; position: relative; min-height: 420px; }
        .gfm-devices {
        ...
        .gfm-map-wrap { position: relative; flex: 1; min-width: 0; }
        .gfm-map { position: absolute; inset: 0; }
    ...
    _initMap() {
      ...
      // Leaflet needs a resize kick once the card has real layout dimensions.
    ...
    _startPlayback() {
      ...
      const baseInterval = 900; // ms per step at 1x
      const tick = () => {
