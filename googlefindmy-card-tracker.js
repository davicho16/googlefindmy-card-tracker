  ...
  class GoogleFindMyCardTracker extends HTMLElement {
    ...
    async _fetchHistory(entityId, range) {
      ...
      const path = `history/period/${start.toISOString()}?filter_entity_id=${encodeURIComponent(
        entityId
      )}&end_time=${encodeURIComponent(end.toISOString())}&no_attributes=false&significant_changes_only=0`;
      let result;
      try {
      ...
      // Fallback: si no vino ningun punto y estamos en modo relativo (rango
      // "hasta ahora", no un rango historico personalizado ya cerrado en el
      // pasado), sintetiza un punto desde el estado en vivo de la entidad
      // para que el mapa nunca quede en blanco.
      if (deduped.length === 0 && isRelative) {
